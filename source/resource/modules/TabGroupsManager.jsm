...
TabGroupsManagerJsm.GlobalPreferences = function ()
{
  try
  {
    ...
    this.__defineGetter__("groupNameRegistered", function ()
    {
      return this._groupNameRegistered;
    });
    this.__defineGetter__("groupNameHistory", function ()
    {
      return this._groupNameHistory;
    });
...
TabGroupsManagerJsm.GlobalPreferences.prototype.observe = function (aSubject, aTopic, aData)
{
  ...
  switch (aData)
  {
  case "bookmarkFolderLClick":
    this.bookmarkFolderLClick = this.prefBranch.getIntPref("bookmarkFolderLClick");
    break;
  case "bookmarkFolderMClick":
    this.bookmarkFolderMClick = this.prefBranch.getIntPref("bookmarkFolderMClick");
    break;
  case "bookmarkFolderDblClick":
    this.bookmarkFolderDblClick = this.prefBranch.getIntPref("bookmarkFolderDblClick");
    break;
  ...
  case "windowCloseWhenLastGroupClose":
    this.windowCloseWhenLastGroupClose = this.prefBranch.getBoolPref("windowCloseWhenLastGroupClose");
    break;
  case "suspendWhenFirefoxClose":
    this.suspendWhenFirefoxClose = this.prefBranch.getBoolPref("suspendWhenFirefoxClose");
    break;
  case "debug":
    this.debug = this.prefBranch.getBoolPref("debug");
...
TabGroupsManagerJsm.GlobalPreferences.prototype.convertKeyBindCommandToCode = function (command)
{
  ...
  case "OpenNewGroup":
    return 0;
  case "OpenNewGroupActive":
    return 1;
  case "OpenNewGroupRename":
    return 2;
  case "OpenNewGroupRenameActive":
    return 3;
  case "OpenNewGroupHome":
    return 4;
  case "OpenNewGroupHomeActive":
    return 5;
  case "SleepActiveGroup":
    return 10;
  case "RestoreLatestSleepedGroup":
    return 11;
  case "SleepingGroupList":
    return 12;
  case "CloseActiveGroup":
    return 20;
  case "RestoreLatestClosedGroup":
    return 21;
  case "ClosedGroupList":
    return 22;
  case "SuspendActiveGroup":
    return 30;
  case "SelectLeftGroup":
    return 40;
  case "SelectRightGroup":
    return 41;
  case "SelectLeftTabInGroup":
    return 44;
  case "SelectRightTabInGroup":
    return 45;
  case "DisplayHideGroupBar":
    return 50;
  case "ActiveGroupMenu":
    return 60;
  case "GroupBarMenu":
    return 61;
  }
...
TabGroupsManagerJsm.GlobalPreferences.prototype.exportSettings = function (file)
{
  ...
  data[0] = {
    type: TabGroupsManagerJsm.constValues.settingsType
  };
  for (let i = 0; i < nameList.length; i++)
  {
    let oneData = {};
    ...
    case this.prefBranch.PREF_INT:
      oneData.value = this.prefBranch.getIntPref(nameList[i]);
      break;
    case this.prefBranch.PREF_BOOL:
      oneData.value = this.prefBranch.getBoolPref(nameList[i]);
      break;
    case this.prefBranch.PREF_STRING:
      oneData.value = this.prefBranch.getComplexValue(nameList[i], Ci.nsISupportsString).data;
      break;
    }
    data[i + 1] = oneData;
  }
  ...
  try
  {
    ...
    if (data[0].type == TabGroupsManagerJsm.constValues.settingsType)
    {
      for (let i = 1; i < data.length; i++)
      {
        ...
        {
        case this.prefBranch.PREF_INT:
          this.prefBranch.setIntPref(data[i].name, data[i].value);
          break;
        case this.prefBranch.PREF_BOOL:
          this.prefBranch.setBoolPref(data[i].name, data[i].value);
          break;
        case this.prefBranch.PREF_STRING:
...
TabGroupsManagerJsm.SaveData.prototype.sessionBackupByTimerChange = function ()
{
  const event = {
    ...
    {
      TabGroupsManagerJsm.saveData.backupByTimer();
    }
  }
...
TabGroupsManagerJsm.NsIFileWrapper = function (aNsIFile, aCannotAppend)
{
  ...
  this.__defineGetter__("existsAndIsFolder", function ()
  {
    return this.exists && this.isFolder;
  });
  this.__defineGetter__("existsAndIsFile", function ()
  {
    return this.exists && this.isFile;
  });
  this.__defineGetter__("lastModifiedTime", function ()
  {
    return this.nsIFile.lastModifiedTime;
  });
};
...
TabGroupsManagerJsm.QuitApplicationObserver.prototype.observe = function (aSubject, aTopic, aData)
{
  ...
  case "quit-application-requested":
    this.quitApplicationRequested();
    break;
  case "quit-application-granted":
    this.quitApplicationGranted();
    break;
  case "quit-application":
